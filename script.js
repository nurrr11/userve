    ...
    try {
        ...
        if (data.success && data.pendingUsers && data.pendingUsers.length > 0) {
            tbody.innerHTML = data.pendingUsers.map(user => {
                const userId = user.Student_ID || user.Organizer_ID;
                const userName = user.Student_FullName || user.Organizer_Name;
                const userEmail = user.Student_Email || user.Organizer_Email;
                const userContact = user.Student_ContactNumber || user.Organizer_ContactNumber;
                ...
                return `
                    <tr>
                        ...
                        <td>${userEmail}</td>
                        <td>${userContact}</td>
                        <td><span class="badge badge-warning">${userRole}</span></td>
                        <td>
                            <button class="btn btn-primary btn-sm" onclick="approveUser('${userId}', '${userRole}')">Approve</button>
                            <button class="btn btn-danger btn-sm" onclick="rejectUser('${userId}', '${userRole}')">Reject</button>
                        </td>
                    </tr>
                `;
            }).join('');
        } else {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">No pending user approvals found.</td></tr>`;
        }
    } catch (error) {
